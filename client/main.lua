ESX                           = nil
local areas = {
	{x = 2494.68, y = 4855.64, z = 32.56, radio = 20.5,label = "Huerto de Cocaina",id = 439,colour = 71},
	{x = 340.28, y = 6627.64, z = 28.88, radio = 28.5,label = "Huerto de Cocaina",id = 439,colour = 71},
	{x = 1955.84, y = 4796.2, z = 42.52, radio = 15.5,label = "Huerto de Cocaina",id = 439,colour = 71},
}
local locales = {
...
AddEventHandler('tm1_farm:plant', function(cocaseed)
	local a = 0
	for i,v in pairs(areas) do
		if GetDistanceBetweenCoords(v.x, v.y, v.z, GetEntityCoords(GetPlayerPed(-1),true)) <= v.radio then
			...
			if min >= 2 then
				TriggerServerEvent('tm1_farm:planted',cocaseed,x, y, z)
...
AddEventHandler('tm1_farm:removePlanted', function(x,y,z,obj, Plants)
...
function errorarea(msg,temp)
	TriggerEvent("rxtr_notificacion:SetQueueMax", "center", 2)
    TriggerEvent("rxtr_notificacion:SendNotification", {
        text = msg,
        type = "error",
        timeout = temp,
        layout = "centerRight",
